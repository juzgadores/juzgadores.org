        ...
        <div
          className={` ${
            direction === "up" || direction === "down"
              ? "absolute left-full top-1/2 z-10 ml-2 -translate-y-1/2 rounded bg-gray-800 px-2 py-1 text-sm text-white"
              : "absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 rounded bg-gray-800 px-2 py-1 text-sm text-white"
          } `}
        >
          {text}
        </div>
    ...
    >
      ...
      >
        {actionButtons.map((action, index) => (
          <Tooltip key={index} text={action.label} direction={direction}>
            <button
              key={index}
